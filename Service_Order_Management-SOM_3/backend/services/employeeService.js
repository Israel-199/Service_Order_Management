...
class EmployeeService {
 ...
 async getAllEmployees(query) {
  ...
  const allowedSortFields = ['employees_id', 'name', 'email', 'phone', 'specification'];
  const sortByValidated = allowedSortFields.includes(sortBy) ? sortBy : 'employees_id';
  ...
  const { rows: employees, count: total } = await Employee.findAndCountAll({
    ...
    order: [[sortByValidated, sortOrder]],
    limit,
    offset,
  });
