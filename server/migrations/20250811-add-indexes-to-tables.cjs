...
async function up(queryInterface) {
    ...
    async function createIndexIfNotExists(queryInterface, table, indexName, columns) {
    ...
    `, {
        ...
        if (indexes.length === 0) {
            await queryInterface.sequelize.query(`CREATE INDEX CONCURRENTLY ${indexName} ON ${table} (${columns.join(', ')});`);
        }
        else {
    ...
    // await createIndexIfNotExists(queryInterface, 'notifications', 'idx_notifications_employee_id', ['employee_id']);
...
async function down(queryInterface) {
    ...
    // await dropIndexIfExists('notifications', 'idx_notifications_employee_id');
    await dropIndexIfExists('notifications', 'idx_notifications_service_order_id');
}
//# sourceMappingURL=20250811-add-indexes-to-tables.js.map
