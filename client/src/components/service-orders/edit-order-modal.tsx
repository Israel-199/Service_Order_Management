...
import { updateServiceOrderSchema, type UpdateServiceOrder, type Customer, type Employee, type ServiceOrderWithDetails } from "@shared/schema";
...
export default function EditOrderModal({ open, onOpenChange, order }: EditOrderModalProps) {
	...
	const form = useForm<UpdateServiceOrder>({
	...
	useEffect(() => {
		if (order && open) {
			form.reset({
				...
				isRecurring: order.isRecurring ?? undefined,
				recurringFrequency: (order.recurringFrequency ?? undefined) as any,
	...
	const mergedEmployees = useMemo(() => {
		...
		const assignedId = order?.employeeId;
		if (assignedId && !list.some((e) => e.id === assignedId)) {
			if (order?.employee) {
				list.unshift(order.employee as Employee);
			} else {
				list.unshift({ id: assignedId, name: `Employee #${assignedId}`, email: "", phone: null as any, specialties: null as any, isActive: 1 } as unknown as Employee);
			}
		}
		return list;
	}, [employees, order]);
	...
	const updateOrderMutation = useMutation({
		mutationFn: async (data: UpdateServiceOrder) => {
...
const validUnits = ["days", "weeks", "months"] as const;
const isValidUnit = (unit: any): unit is typeof validUnits[number] => validUnits.includes(unit);

const onSubmit = (data: UpdateServiceOrder) => {
	const submitData: UpdateServiceOrder = {
		...data,
		attachments,
		...
		customFrequencyUnit:
			showCustomFrequency && isValidUnit(customFrequencyUnit)
				? customFrequencyUnit
				: undefined,
	};
