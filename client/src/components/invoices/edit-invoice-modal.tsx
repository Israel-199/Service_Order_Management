...
import { insertInvoiceSchema, type InsertInvoice, type ServiceOrderWithDetails, type Invoice } from "@shared/schema";
...
export default function EditInvoiceModal({ invoice, onSuccess }: { invoice: Invoice; onSuccess: () => void }) {
  ...
  const form = useForm<InsertInvoice>({
    ...
    defaultValues: {
      serviceOrderId: String(invoice.serviceOrderId ?? ""),
      items: (invoice.items as any) || [],
      taxCents: invoice.taxCents ?? 0,
      discountCents: invoice.discountCents ?? 0,
      currency: invoice.currency || "ETB",
      status: (invoice.status as "draft" | "sent" | "paid" | "void") || "draft",
      notes: invoice.notes || "",
      issuedAt: (invoice.issuedAt ? new Date(invoice.issuedAt as unknown as string) : undefined),
      dueAt: (invoice.dueAt ? new Date(invoice.dueAt as unknown as string) : undefined),
    },
  });

  const { fields, append, remove } = useFieldArray({ control: form.control, name: "items" });

  // initialize percents from existing invoice values
  const initialSubtotalCents = useMemo(() => {
    const items = (invoice.items as any) ?? [];
    return items.reduce((sum: number, it: any) => sum + (Number(it.quantity || 0) * Number(it.unitPriceCents || 0)), 0);
  }, [invoice.items]);

  const [discountPercent, setDiscountPercent] = useState<number>(() => {
    if (!initialSubtotalCents) return 0;
    return ((invoice.discountCents ?? 0) / initialSubtotalCents) * 100;
  });

  const [taxPercent, setTaxPercent] = useState<number>(() => {
    const taxable = Math.max(0, initialSubtotalCents - (invoice.discountCents ?? 0));
    if (!taxable) return 15;
    return ((invoice.taxCents ?? 0) / taxable) * 100;
  });
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit((values) => updateMutation.mutate(values))} className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="border-t pt-4 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
          <div className="text-sm text-techflow-secondary">
            Subtotal: <span className="font-medium text-techflow-primary">{formatCents(subtotalCents, form.getValues("currency") || "USD")}</span>
            {" "} | Tax: <span className="font-medium text-techflow-primary">{formatCents(Number(form.watch("taxCents") || 0), form.getValues("currency") || "USD")}</span>
            {" "} | Discount: <span className="font-medium text-techflow-primary">{formatCents(Number(form.watch("discountCents") || 0), form.getValues("currency") || "USD")}</span>
          </div>
          <div className="text-lg font-semibold text-techflow-primary">
            Total: {formatCents(totalCents, form.getValues("currency") || "USD")}
          </div>
        </div>

        <div className="flex justify-end gap-2">
          <Button type="submit" className="bg-primary hover:bg-blue-700" disabled={updateMutation.isPending}>
            Save Changes
          </Button>
        </div>
      </form>
